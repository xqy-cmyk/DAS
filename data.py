...
# ------------------------------------------------------------------------------------------------------------------------------
	...
	#The default label is volumetric_soil_water_layer_20
        ...
        print('label type is {d_p}'.format(d_p=label.dtype))
        lat_min, lat_max = 70, 146#37,72
        lon_min, lon_max = 144, 275#73,135
        #global
        mask = np.ones(label[0,:,:,0].shape)
        mask[np.isnan(label[0, :, :, 0])] = 0
        min_map = np.min(label[:365, :, :, 0], axis=0)  
        max_map = np.max(label[:365, :, :, 0], axis=0)  
        mask[min_map == max_map] = 0
        print("label shape:", label.shape)
        print("mask shape:", mask.shape)
        print("min_map shape:", min_map.shape)
        print("max_map shape:", max_map.shape)
        #china region
#        mask = np.ones(label[0, :, :, 0].shape)
#        mask[:lat_min, :] = 0 
#        mask[lat_max:, :] = 0 
...
#        mask[np.isnan(label[0, :, :, 0])] = 0
        ...
        plt.title('Mask for China Region')
        ...
        if not os.path.exists(PATH + 'static_norm.npy'):
            ...
            for i in range(len(self.static_list)):
                ...
                with xr.open_dataset(file_static) as f_:
                    ...
                    #static_data1 = static_data
                    static_data = self._interp(static_data,mask)   
                static.append(static_data)
                #static1.append(static_data1)
            static = np.stack(static, axis=-1)
            #static1 = np.stack(static1, axis=-1)
     
#            for i in range(static1.shape[-1]): 
#                plt.figure(figsize=(12, 6))
#                plt.imshow(static1[:, :, i], origin='lower', aspect='auto', cmap='viridis')
#                plt.colorbar(label=f'Feature {i} Value')
#                plt.title(f'Spatial Distribution of Feature {i}', fontsize=16)
#                plt.xlabel('Longitude Index', fontsize=12)
#                plt.ylabel('Latitude Index', fontsize=12)
#                plt.show()
           # cluster_data = static1[:, :, :]
            cluster_data = static[:, :, :]
            np.save(PATH + 'cluster_data.npy', cluster_data)
...
# save
        ...
        return x_train_norm, y_train_norm, x_test_norm, y_test_norm, static, lat, lon, mask, cluster_data
#------------------------------------------------------------------------------------------------------------------------------
